$(document).ready(function() {
    ...
    //     for (var v = 0; v < cube.geometry.vertices.length; v++) {

    //         vertices.push(cube.geometry.vertices[v].clone());
    ...
    for (var i = 0; i < cube.geometry.vertices.length; i++) {
        var currVert = cube.geometry.vertices[i];
        // console.log(currVert);
        // axis = ['x', 'y', 'z'];
        for (var j = 0; j < 3; j++) {
            console.log(currVert.x,currVert.y,currVert.z);
        };
        /*for (var i = Things.length - 1; i >= 0; i--) {

        };*/
        // console.log(cube.geometry.vertices[1]);
    };
    ...
    cameraFldr = gui.addFolder('Camera');

    cameraFldr.add(ctrlr, 'camera_pos_x', -500, 500).step(1);
    cameraFldr.add(ctrlr, 'camera_pos_y', -500, 500).step(1);
    cameraFldr.add(ctrlr, 'camera_pos_z', -500, 500).step(1);

    shear = gui.addFolder('CubeShear');

    SyxCtrlr = shear.add(ctrlr, 'Syx', -2.00, 2.00).step(0.01);
    SzxCtrlr = shear.add(ctrlr, 'Szx', -2.00, 2.00).step(0.01);
    SxyCtrlr = shear.add(ctrlr, 'Sxy', -2.00, 2.00).step(0.01);
    SzyCtrlr = shear.add(ctrlr, 'Szy', -2.00, 2.00).step(0.01);
    SxzCtrlr = shear.add(ctrlr, 'Sxz', -2.00, 2.00).step(0.01);
    SyzCtrlr = shear.add(ctrlr, 'Syz', -2.00, 2.00).step(0.01);
